...
class OCRExtractor:
    ...
    def extract_ocr_data(self, preprocessed_image, mode="form"):
        """
        Extract text with positional and confidence data using PaddleOCR.
        
        PaddleOCR is superior for:
        - Handwritten text recognition
        - Mixed printed/handwritten documents
        - Text at various angles
        - Low-quality scans
        ...
        Args:
            preprocessed_image (ndarray): Preprocessed image from preprocess.py
            mode (str): Ignored for PaddleOCR (kept for API compatibility)
        ...
        Returns:
            dict: OCR data dictionary with keys:
                ...
                - width: list of bounding box widths
                - height: list of bounding box heights
                - confidence: list of confidence scores (0-100, converted from 0-1)
                - raw_data: raw PaddleOCR output
        """
        # Unified extraction that works for either backend
        if preprocessed_image is None:
            print("❌ ERROR: Preprocessed image is None")
            return self._empty_ocr_data()
        ...
        if self.engine == 'paddle':
            try:
                ...
                print(f"❌ ERROR during PaddleOCR extraction: {e}")
                import traceback
                traceback.print_exc()
                return self._empty_ocr_data()
            ...
            try:
                ...
                config = f"--psm {self.psm_modes.get(mode,6)} --oem 3"
                raw = self.pytesseract.image_to_data(preprocessed_image, config=config, output_type=Output.DICT)
                ...
                for i, conf in enumerate(raw.get('conf', [])):
                    try:
                    ...
                    if confidence > 0:
                        text = raw.get('text', [])[i].strip()
                        if text:
                            processed_data['text'].append(text)
                            processed_data['left'].append(int(raw.get('left', [0])[i]))
                            processed_data['top'].append(int(raw.get('top', [0])[i]))
                            processed_data['width'].append(int(raw.get('width', [0])[i]))
                            processed_data['height'].append(int(raw.get('height', [0])[i]))
                ...
                print(f"❌ ERROR during pytesseract extraction: {e}")
                import traceback
                traceback.print_exc()
                return self._empty_ocr_data()

        else:
            print("❌ No OCR engine initialized. Please install 'paddleocr' or 'pytesseract'.")
            return self._empty_ocr_data()
    ...
    def extract_plain_text(self, preprocessed_image, mode="form"):
        ...
        Args:
            preprocessed_image (ndarray): Preprocessed image
            mode (str): Ignored for PaddleOCR
